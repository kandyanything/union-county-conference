...
// The roster comes from data/schools.json, which lists every member school.
// It deliberately does NOT come from scripts/ds-schools.json: that file is a
// list of DigitalSports scrape targets, so a school that moves to ArbiterLive
// or to its own iCal feed drops out of it and would silently stop being
// recognised in the news. Kent Place, Oak Knoll and Oratory Prep are all in
// that position.
const rosterFile = path.join(ROOT, 'data', 'schools.json');
const ROSTER = (() => {
    if (!fs.existsSync(rosterFile)) return [];
    const parsed = JSON.parse(fs.readFileSync(rosterFile, 'utf8').replace(/^﻿/, ''));
    return Array.isArray(parsed) ? parsed : (parsed.schools || []);
})();
const SCHOOL_NAMES = ROSTER.map(s => s.name).filter(Boolean);
...
const GENERIC_ALIASES = new Set([
    'academy', 'campus', 'catholic', 'central', 'park', 'prep', 'regional', 'tech',
]);
...
const LOCAL_CONTEXT_RE = /\b(Union County|UCIAC|Westfield|Cranford|Scotch Plains)\b/i;
...
const HOME_TOWNS = new Set([
    'berkeley', 'clark', 'cranford', 'elizabeth', 'fanwood', 'garwood',
    'hillside', 'kenilworth', 'linden', 'mountainside', 'plainfield', 'rahway',
    'roselle', 'scotch', 'springfield', 'summit', 'union', 'westfield',
]);
...
// This conference IS the county, and the press writes it that way: "Union
// County girls soccer title", "Union County tournament seeds". Those are our
// readers' stories even when no single school is named. The exclusions are the
// other things that carry the county's name and are nothing to do with us.
const COUNTY_RE = /\bUnion County\b/i;
const COUNTY_FP = /Union County (College|Vocational|Vo-?Tech|Freeholder|Commissioner|Prosecutor|Sheriff|Police|Clerk|Board of)/i;

function matchRelevance(text) {
    const shortHit = new RegExp(`\\b${escapeRe(CONF_SHORT)}\\b`).test(text)
        && !COUNTIES_FP.test(text);
    const countyHit = COUNTY_RE.test(text) && !COUNTY_FP.test(text);
    const confHit = phraseRe(CONF_NAME).test(text) || shortHit || countyHit;
